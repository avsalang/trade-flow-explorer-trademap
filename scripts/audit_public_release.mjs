...
const tradePath = path.join(publicDataDirectory, "trade-flows-trademap.json");
...
if (!trade.reportingViews?.imports || !trade.reportingViews?.exports) {
  throw new Error("Both import and export views are required.");
}
if (
  trade.meta?.source !== "ITC Trade Map" ||
  trade.meta?.reporterScope !== "Asia-Pacific (ATO)" ||
  trade.meta?.partnerScope !== "World"
) {
  throw new Error("TradeMap source and geographic scope metadata are incomplete.");
}
...
for (const [mode, view] of Object.entries(trade.reportingViews)) {
  ...
  view.records.forEach((record) => {
    if (
      ...
      record.length !== 5 ||
      record[3].length !== trade.years.length ||
      ![0, 1].includes(record[4])
    ) {
  ...
  view.snapshotSummaries.forEach((productSummaries) => {
    ...
    productSummaries.forEach((summary) => {
      ...
      if (
        ...
        !Number.isInteger(summary.directReporterCount) ||
        !Number.isInteger(summary.mirrorReporterCount)
      ) {
...
const configuredRouteLimit = trade.meta.publication.topBilateralRoutesPerProductYear;
const maximumTradeValues =
  trade.products.length * trade.years.length * configuredRouteLimit * 2;
if (activeTradeValues > maximumTradeValues) {
  throw new Error(
    `Public trade extract exposes ${activeTradeValues.toLocaleString()} active values; ` +
      `configured limit is ${maximumTradeValues.toLocaleString()}.`,
  );
}
...
console.log(`All public records: ${publicRecordCount.toLocaleString()}`);
