...
function defaultTrendRange(years, product) {
  const finalYear = years.includes(DEFAULT_YEAR) ? DEFAULT_YEAR : Math.max(...years);
...
function reportingBasisLabel(value) {
  if (value === 0) return "Direct";
  if (value === 1) return "Mirror estimate";
        ...
        <tbody>
          {rows.map((row, index) => (
            <tr key={row.id} onClick={() => onSelect(row.id)}>
              ...
              <td><span className="basis-badge">{row.basisLabel}</span></td>
            </tr>
          ))}
        </tbody>
      </table>
...
function buildSnapshotAnalysis(
  ...
  const fullPartnerTotals = new Map();
  const reporterHhiParts = new Map();
  const reporterIndexes = new Set();
  const directReporterIndexes = new Set();
  const mirrorReporterIndexes = new Set();
  ...
  summaries.forEach((summary) => {
    ...
    const validPartnerTotals = (summary.partnerTotals || []).filter(
      ([partnerIndex]) => !isSpecialEconomy(dataset.economies[partnerIndex]),
    );
    addEntries(fullPartnerTotals, validPartnerTotals);
    if (reporterEconomyIndex === null) addEntries(partnerTotals, validPartnerTotals);
    ...
    (summary.reporterBasis || []).forEach(([reporterIndex, basisFlag]) => {
      if (
        reporterEconomyIndex !== null &&
        reporterIndex !== reporterEconomyIndex
      ) return;
      if (basisFlag === 1) mirrorReporterIndexes.add(reporterIndex);
      else directReporterIndexes.add(reporterIndex);
    });
  });
  ...
  if (publishedGroupSnapshot && reporterEconomyIndex === null) {
    ...
    view.records.forEach(([productIndex, importerIndex, exporterIndex, values, basisFlag]) => {
      ...
      row.value += value;
      if (basisFlag === 0 || basisFlag === 1) row.basisFlags.add(basisFlag);
    ...
    leadingRouteTotals.forEach((value, key) => {
      ...
      if (!published || published.value < value) {
        routeTotals.set(key, {
          ...
          importerIndex,
          value,
          basisFlags: published?.basisFlags || new Set(),
        });
      }
    });
  ...
  const routes = [...routeTotals.values()]
    .map((row) => {
      ...
      const basisFlag = row.basisFlags?.size === 1
        ? [...row.basisFlags][0]
        : null;
      return {
        ...row,
        ...
        basisFlag,
        basisLabel: reportingBasisLabel(basisFlag),
      };
    })
    .sort((left, right) => right.value - left.value);
      ...
      mapNodeTotals.set(economyIndex, {
        supplierValue:
          reportingMode === "exports"
            ...
            : fullPartnerTotals.get(economyIndex) || 0,
        importerValue:
          reportingMode === "exports"
            ? fullPartnerTotals.get(economyIndex) || 0
            : reporterTotals.get(economyIndex) || 0,
  ...
  return {
    ...
    directReporterCount: directReporterIndexes.size,
    mirrorReporterCount: mirrorReporterIndexes.size,
    concentrationAvailable,
    overallHhi,
...
export default function TradeExplorerApp() {
  ...
  useEffect(() => {
    fetch(assetUrl("data/trade-flows-trademap.json"))
      .then((response) => {
      ...
      .then((payload) => {
        ...
        setYear(payload.years.includes(DEFAULT_YEAR) ? DEFAULT_YEAR : Math.max(...payload.years));
      })
  ...
  useEffect(() => {
    ...
    if (year < availableFrom) {
      setYear(dataset.years.includes(DEFAULT_YEAR) ? DEFAULT_YEAR : Math.max(...dataset.years));
    }
  ...
  const resetFilters = () => {
    ...
    setYear(dataset?.years.includes(DEFAULT_YEAR) ? DEFAULT_YEAR : Math.max(...(dataset?.years || [DEFAULT_YEAR])));
  ...
  const mirrorCoverageText = analysis?.mirrorReporterCount
    ? `Includes mirror estimates for ${analysis.mirrorReporterCount} ${analysis.mirrorReporterCount === 1 ? "economy" : "economies"}`
    : "Direct reports only";
  ...
  const hasFilters =
    ...
    (analysisView === "trends"
      ...
      : year !== (dataset.years.includes(DEFAULT_YEAR) ? DEFAULT_YEAR : Math.max(...dataset.years)) ||
                  ...
                  {[...dataset.years].reverse().map((item) => {
                    ...
                    return (
                      <option key={item} value={item} disabled={unavailable}>
                        ...
                        {unavailable
                          ? " · unavailable"
                          : dataset.meta?.incompleteYears?.includes(item)
                            ? " · Incomplete coverage"
                            : ""}
                      </option>
                    );
                  ...
                  {eligibleTrendYears
                    ...
                    .map((item) => (
                      <option key={item} value={item}>
                        {item}{dataset.meta?.incompleteYears?.includes(item) ? " · Incomplete coverage" : ""}
                      </option>
                    ))}
                </select>
                <ChevronDown size={15} />
        ...
        <div className="content">
          ...
          <section className="kpi-grid" aria-label="Headline indicators">
            <KpiCard
              ...
              subtext={countryEconomy
                ? `${countryEconomy.name} World total · ${mirrorCoverageText}`
                : `World totals from ${analysis.reporters} Asia-Pacific economies · ${mirrorCoverageText}`}
            />
            <KpiCard
          ...
          <div hidden={flowView !== "table"}>
            <Panel
              ...
              subtitle={`${year} · Asia-Pacific ${reportingMode} with worldwide partners`}
          ...
          >
            <div className="sankey-basis-note">
              Includes the full bilateral value for the selected Asia-Pacific scope. The route and economy filters apply only to the map and table.
            </div>
            <TradeSankey
              routes={sankeyFlows}
          ...
          <div className="two-column-grid">
            <Panel
              ...
              subtitle={countryEconomy ? `${countryEconomy.name} World total` : "Reported World totals"}
            >
              <RankingBars
            ...
            <Panel
              ...
              subtitle={countryEconomy ? `Partners reported by ${countryEconomy.name}` : `Bilateral ${reportingMode} with worldwide partners`}
            >
              <RankingBars
          ...
          <footer className="page-footer">
            <div>
              ...
              <span>
                Source: {dataset.meta?.source || "ITC Trade Map"}. Annual {reportingMode} by Asia-Pacific reporting economies with worldwide partners, in current US$. Direct and mirror estimates are identified in the route table.
              </span>
            </div>
          </footer>
          </>
